...
def register_task(task_id: str, task: asyncio.Task, cancel_event: asyncio.Event) -> None:
...
async def test_ai(req: AnalyzeRequest):
    ...
    try:
        ...
        return {
            ...
            "provider": req.settings.ai_provider, "model": usage.get("model"),
            "tokens": usage, "raw_response": raw, "raw_length": len(raw),
            "first_chars": repr(raw[:100]),
            "json_parse_ok": parse_ok, "json_parse_error": parse_error, "parsed": parsed,
        }
    except Exception as e:
        elapsed = round(time.monotonic() - t0, 2)
        tb      = traceback.format_exc()
        print(f"[test-ai] ✗ FAILED after {elapsed}s:\n{tb}")
        return {"success": False, "elapsed_s": elapsed, "provider": req.settings.ai_provider, "error": str(e), "traceback": tb}
...
async def _do_analyze(req: AnalyzeRequest, cancel_event: asyncio.Event | None = None) -> dict:
    ...
    print(f"[analyze]   → calling AI | mode={'vision' if vision else 'standard'} | images={len(images)} | prompt={len(prompt)} chars")
    t2    = time.monotonic()
