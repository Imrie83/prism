...
app.get("/health", (req, res) => {
  res.json({ status: "ok", service: "prism-screenshot" });
});
...
app.post("/screenshot", async (req, res) => {
  ...
  try {
    ...
    console.log(`[screenshot] Done in ${elapsed}s — ${Math.round(result.screenshot.length / 1024)}KB`);
    res.json(result);
  } catch (err) {
...
app.post("/screenshot-offset", async (req, res) => {
  ...
  try {
    ...
    if (clipHeight <= 0) { await browser.close(); return res.json({ screenshot: null, offset_y, pageHeight }); }
    const buf = await page.screenshot({ clip: { x: 0, y: offset_y, width: 1280, height: clipHeight }, type: "jpeg", quality: 55, animations: "disabled" });
    await browser.close();
    const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
    console.log(`[screenshot-offset] Done in ${elapsed}s — ${Math.round(buf.length / 1024)}KB`);
...
app.listen(PORT, () => {
  console.log(`Prism screenshot service running on http://localhost:${PORT}`);
});
